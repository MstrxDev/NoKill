...
- **Vault retention** — the vault cannot grow unbounded: after each preserve, oldest entries are pruned to stay within caps (defaults: 2 GB total, 200 entries; age-based pruning exists but is off by default — age alone never deletes evidence). Pruning only ever touches the vault's own entry folders, never the entry just written, and never user files; pruned entries are reported, and the dashboard's history tab shows the current vault footprint.
